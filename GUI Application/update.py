...
class WaveformPanel(QScrollArea):
    def __init__(self, parent=None):
        ...
        self.time_ms: np.ndarray = np.array([])
    ...
    def show_merged(self, selected_rows: list[dict]):
        ...
        for row in selected_rows:
            ...
            plot.plot(self.time_ms, row["pcm"], pen=pen, antialias=True, name=row["label"])
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self._table.setStyleSheet(f"QTableWidget {{ alternate-background-color: #1e2128; }}")
    ...
    def _open_file(self):
        ...
        try:
            ...
            raw_times = raw.iloc[0, PCM_COL_START:].values.astype(float)
            mask = np.isfinite(raw_times)
    ...
    def _on_check_changed(self, item: QTableWidgetItem):
        ...
        # If we're in merged view, stay in merged view and update it
    ...
    def _rebuild_waveforms(self):
        if self._df is None:
            return
        self._wave_panel.rebuild(self._selected_rows_data())
    ...
    def _show_merged(self):
        ...
        self._wave_panel.show_merged(self._selected_rows_data())
